...
public class ThreadedStructure implements GroupBy, Applyable {
    String[] titles;
    ...
    ThreadedStructure(HashMap<String, ArrayList<ArrayList>> listDF, String[] kol, String[] oldTitles, Class<? extends Value>[] oldTypes, ArrayList<String> zbior_id) {
    ...
    public DataFrame helper(String operation) { // za jego pomocą implementuje min, max, std, itd...
        ExecutorService executor = Executors.newFixedThreadPool(10);
        ArrayList<Future<DataFrame>> results = new ArrayList<>(); // najpierw na listach
        ...
        int it=0;
        while(it < hashMapaDF.size()){
            results.add(executor.submit(new OperationOnDf(operation,hashMapaDF.get(it))));
            it++;
        }
